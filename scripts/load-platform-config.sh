...
# Sets: ARCH_NAME, TARGET, SYSROOT_NAME, DOCKER_PLATFORM, BASE_IMAGE, BUILDER_SYSROOT_DOCKERFILE, CMAKE_PROCESSOR, CMAKE_FLAGS
...
while IFS='|' read -r arch target sysroot platform base_image builder_sysroot_dockerfile cmake_proc cmake_flags rest; do
    ...
    if [ "$arch" = "$ARCH_NAME" ]; then
        ...
        export BUILDER_SYSROOT_DOCKERFILE="$builder_sysroot_dockerfile"
